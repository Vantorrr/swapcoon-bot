...
bot.on('callback_query:data', async (ctx) => {
    ...
    if (data.startsWith('custom_details_')) {
        ...
        await ctx.reply(
            ...
            `🆔 Заказ #${orderId}\n\n` +
            `📝 Напишите реквизиты в свободной форме.\n` +
            `Можно отправить одним или несколькими сообщениями.`,
            { 
                parse_mode: 'HTML',
                reply_markup: new InlineKeyboard()
    ...
    if (data.startsWith('funds_sent_')) {
        ...
        try {
            ...
            await ctx.api.sendMessage(order.client_id,
                ...
                `🔍 Отслеживайте поступление средств\n\n` +
    ...
    // Обработка оценки качества обслуживания
    if (data.startsWith('rate_service_')) {
...
bot.command('ref', async (ctx) => {
    ...
    await ctx.reply(
        ...
        `🎯 Поделитесь ссылкой с друзьями\n` +
                    `💸 За каждый их обмен получаете 0.2%\n` +
    ...
    if (chatContexts.has(userId)) {
        ...
        if (context.action === 'input_custom_details') {
            ...
            try {
                ...
                console.log('✅ РЕКВИЗИТЫ УСПЕШНО ОТПРАВЛЕНЫ КЛИЕНТУ И ОПЕРАТОРУ');
                chatContexts.delete(userId);
                return;
