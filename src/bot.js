...
async function createMainKeyboard(userId) {
    ...
    try {
        ...
        // ВРЕМЕННЫЙ ОБХОД: Если пользователь с ID 8141463258 - даем админ права
        if (userId === 8141463258) {
            console.log('🛡️ ВРЕМЕННЫЙ ДОСТУП: Главный админ обнаружен');
        ...
        if (userId === 8141463258) {
            console.log('🛡️ АВАРИЙНЫЙ ДОСТУП: Добавляем админ кнопки для главного админа');
            keyboard.text('🛡️ Админ панель', 'open_admin_panel')
                .text('👨‍💼 Панель оператора', 'open_operator_panel')
                .row();
        }
...
bot.command('start', async (ctx) => {
    ...
    await ctx.replyWithPhoto('https://i.ibb.co/hP0PbvF/IMAGE-2025-07-09-12-57-19.jpg', {
        caption: `🦝 <b>Добро пожаловать в SwapCoon!</b>\n\n` +
            `🚀 Самый быстрый и безопасный сервис обмена валют\n\n` +
            `<b>🔥 Что умеет наш енот:</b>\n` +
...
bot.on('callback_query:data', async (ctx) => {
    ...
    if (data === 'webapp_launch') {
        try {
            ...
            await ctx.reply(
                ...
                '🌟 Полнофункциональное приложение для обмена валют\n' +
    ...
    if (data === 'back_to_main') {
        ...
        await ctx.replyWithPhoto('https://i.ibb.co/hP0PbvF/IMAGE-2025-07-09-12-57-19.jpg', {
            caption: `🦝 <b>Добро пожаловать обратно в SwapCoon!</b>\n\n` +
                `🚀 Самый быстрый и безопасный сервис обмена валют\n\n` +
                `<b>🔥 Что умеет наш енот:</b>\n` +
                `💱 Обмен криптовалют (42 пары)\n` +
                `💵 Обмен наличных в офисах\n` +
